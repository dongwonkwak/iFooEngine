...
namespace foo::net
{
	std::optional<ipv6_address> ipv6_address::from_string(std::string_view string) noexcept
	{
	...
	std::string ipv6_address::to_string() const
	{
		...
		{
			if (bytes_[2 * i] == 0 && bytes_[2 * i + 1] == 0)
			{
				const std::uint32_t zeroRunStart = i;
				++i;
				while (i < 8 && bytes_[2 * i] == 0 && bytes_[2 * i + 1] == 0)
				{
					++i;
				}
		...
		{
			const std::uint8_t highByte = bytes_[index * 2];
			const std::uint8_t lowByte = bytes_[index * 2 + 1];
